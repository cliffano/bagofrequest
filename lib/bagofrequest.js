 ...
 *            { statusCodes: [ 503 ], scale: 500, delay: 1000, maxRetries: 10 }
...
function req(method, url, opts, cb) {
  ...
  var requestMethod = _requestMethod(method);
  request[requestMethod](params, _handleRetry(opts, requestMethod, params, function (err, result) {
    if (err) {
      cb(err);
    } else {
...
function _handleRetry(opts, method, params, cb) {
  if (!opts.retry) return cb;

  var retryCount = 1;
  var defaults = { statusCodes: [ 408, 502, 503, 504 ], scale: 500, delay: 1000, maxRetries: 10 };
  var retry = opts.retry === true ? defaults : opts.retry;
  retry.maxRetries = retry.maxRetries >= 0 ? retry.maxRetries : NaN;
  retry.statusCodes = retry.statusCodes || [];

  function retryRequest() {
    request[method](params, retryCallback);
  }

  function retryCallback(err, result) {
    if (result && retry.statusCodes.indexOf(result.statusCode) !== -1) {
      if (retryCount >= retry.maxRetries) {
        (result = (result || {})).retryLimitHit = true;
        cb(err, result);
      } else {
        retryCount++;
        retryRequest();
