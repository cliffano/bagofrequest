 ...
 *            { retryOnSocketError: true, statusCodes: [ 503 ], scale: 0.5, delay: 1000, maxRetries: 10 }
...
function _execRequest(opts, method, params, cb) {
  ...
  var tryCount = 0,
    ...
    retryOpts = (opts.retry === true) ?
      DEFAULTS :
      {
        retryOnSocketError: (opts.retry.retryOnSocketError !== undefined) ? opts.retry.retryOnSocketError : DEFAULTS.retryOnSocketError,
  ...
  function does(cb) {
    request[method](params, function (err, result) {
      ...
      retryable = opts.retry &&
        tryCount <= retryOpts.maxRetries &&
        ((retryOpts.retryOnSocketError && err) ||
        (_wildcardMatch(result.statusCode, retryOpts.statusCodes) !== undefined));
