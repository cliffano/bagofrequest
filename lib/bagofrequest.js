...
function _execRequest(opts, method, params, cb) {
  ...
  var retryCount = 0,
    retryOpts, currDelay, lastResult, retryable;
  ...
  function does(cb) {
    request[method](params, function (err, result) {
      ...
      lastResult = result;
      retryable = opts.retry && _wildcardMatch(result.statusCode, retryOpts.statusCodes);

      if (retryable) {
        retryCount++;
        if (retryCount === retryOpts.maxRetries - 1) {
  ...
  function check() {
    return retryable && retryCount < retryOpts.maxRetries;
  }
  ...
  function done(err) {
    if (!err && lastResult) {
      lastResult._retry = {
        ...
        retryLimitHit: (retryCount === retryOpts.maxRetries)
      };
    }
    cb(err, lastResult);
  }
