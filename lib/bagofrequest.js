...
function _execRequest(opts, method, params, cb) {
  const DEFAULTS = {
    errorCodes: [
      ...
      'EIO', 'ENETDOWN', 'ENETRESET', 'ENETUNREACH', 'ETIMEDOUT'
    ],
  ...
  var tryCount = 0,
    ...
    retryOpts = (opts.retry === true) ?
      DEFAULTS :
      {
        errorCodes: (opts.retry.errorCodes !== undefined) ? opts.retry.errorCodes : DEFAULTS.errorCodes,
  ...
  function does(cb) {
    request[method](params, function (err, result) {
      var retryableError, retryableResult;

      tryCount++;
      lastResult = result;
      retryableError = err && (retryOpts.errorCodes === true ||
        (typeof(retryOpts.errorCodes) === 'object' && retryOpts.errorCodes.indexOf(err.code) !== -1));
